...
4. Extract the ZIP directly into the domain's document root. The ZIP is packaged root-ready: `index.html`, `.htaccess`, `contact.php`, `en/`, `zh/`, and `assets/` should sit directly in that root. Do not leave the site one folder too deep.
...
6. Before changing public DNS, test `/en/`, `/zh/`, `/en/new-service/`, `/zh/new-service/`, images, mobile navigation, and the contact form. A local hosts-file override is preferred because `.htaccess` canonicalises other hostnames to the live domain. If a temporary staging hostname is used, temporarily disable only the hostname canonicalisation rule and restore it before launch.
...
3. Keep the included `.nojekyll` file so GitHub serves the files directly without Jekyll processing.
